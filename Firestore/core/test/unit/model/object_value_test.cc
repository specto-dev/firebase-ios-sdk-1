...
namespace {
bool operator==(const absl::optional<google_firestore_v1_Value>& lhs,
                const absl::optional<google_firestore_v1_Value>& rhs) {
  return (!lhs.has_value() && !rhs.has_value()) ||
         (lhs.has_value() && rhs.has_value() && Values::Equals(*lhs, *rhs));
}
...
TEST_F(ObjectValueTest, ExtractsFields) {
  ...
  // TODO(mrschmidt): Why does EXPECT_EQ not work?
...
TEST_F(ObjectValueTest, AddsNewFields) {
  ...
  object_value.Set(Field("b"), Wrap(1));
  EXPECT_EQ(WrapObject("a", "object_value", "b", 1), object_value);
}
...
TEST_F(ObjectValueTest, AddsMultipleFields) {
  ...
  object_value.SetAll(FieldMask({Field("a"), Field("b")}),
                      WrapObject("a", "object_value", "b", 1));
  EXPECT_EQ(WrapObject("a", "object_value", "b", 1), object_value);
}
...
TEST_F(ObjectValueTest, ImplicitlyCreatesObjects) {
  ...
  EXPECT_EQ(
      ...
      object_value)
      << Values::CanonicalId(*object_value.Get({}));  // Add ToString
}
...
TEST_F(ObjectValueTest, DeletesMultipleKeys) {
  ...
  object_value.SetAll(FieldMask({Field("a"), Field("b")}), WrapObject());
